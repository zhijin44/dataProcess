...
logging.basicConfig(filename="log/SAGE_bn_seperate.log",
...
# Load the dataset
# dataset = torch.load('dataset_per.pt')
dataset = torch.load('dataset/dataset_con.pt')
...
model = GCNModel(num_node_features=2, hidden_channels=[256, 128, 32], num_classes=1)
...
for epoch in tqdm(range(401)):
    ...
    if epoch % 10 == 0:
        ...
        logging.info(f"Epoch: {epoch}, Train Loss: {train_loss}, Train Accuracy: {train_accuracy}, Test Loss: {test_loss}, "
            f"Test Accuracy: {test_accuracy}")